 ...
 *   - `publicAuthServerURL`  Public URL for you Realm authorization endpoint, in the case where this is different.
...
function Strategy(options, verify) {
  ...
  const publicUrl = options.publicAuthServerURL || options.authServerURL;

  options.authorizationURL = `${publicUrl}/realms/${options.realm}/protocol/openid-connect/auth`;
  options.tokenURL = `${options.authServerURL}/realms/${options.realm}/protocol/openid-connect/token`;
  console.debug({
    auth: options.authorizationURL,
